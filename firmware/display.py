...
#
# Buttons:
#   KEY A  (GP15)  toggle backlight  (full -> dim -> off -> full)
...
def _poll_buttons():
    ...
    if not _btn:
        return
    now = time.ticks_ms()
    if time.ticks_diff(now, _last_btn_ms) < 120:    # debounce / repeat guard
        return
    for name, pin in _btn.items():
        val = pin.value()
        if _prev[name] == 1 and val == 0:           # falling edge = press
            _last_btn_ms = now
            # Page navigation: KEY A / KEY B (reliable), OR joystick L/R.
            if name in ("b", "right"):
                _page = (_page + 1) % _NPAGES
            elif name in ("a", "left"):
                _page = (_page - 1) % _NPAGES
            elif name == "up":
                _bl_level = (_bl_level + 1) % 3      # joystick UP cycles backlight
                _apply_backlight()
            elif name == "down":
                _reset_stats()                       # joystick DOWN resets stats
            elif name == "ctrl":
                global _hold
                _hold = not _hold                    # joystick PRESS = hold/freeze
            _dirty_set()
        _prev[name] = val
...
def _header(title):
    ...
    tag = "HOLD" if _hold else "%d/%d" % (_page + 1, _NPAGES)
    _text(tag, W - 32, 3, BLACK)
