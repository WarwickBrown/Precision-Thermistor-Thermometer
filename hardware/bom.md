...
| LCD1 | Waveshare Pico-LCD-1.14 V2 | 240×135 ST7789, joystick + 2 keys | stacks on the Pico header |
...
| (none) | Stripboard (Veroboard) | | analog + digital sections, star ground |
| (none) | Shielded twisted-pair cable | mic cable or similar | jack to thermistor runs |
...
| U3, J3, R_pu1 | DS18B20 + 1-Wire jack + 4.7 kΩ pull-up | ambient 1-Wire sensor. Drawn in the KiCad schematic (`ADS1115BasicDesign`) as an optional addition, but not fitted in the build shown here. The firmware reads it if present (the `t_amb_c` column) and skips it otherwise |
